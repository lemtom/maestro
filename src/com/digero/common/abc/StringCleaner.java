...
public class StringCleaner {
	...
	public static String cleanForFileName(String before) {
	    ...
	    if (after.length() == 0) after = "mySong";
	    return after;
	}
	...
	private static String convertCyrillic (String before) {
		// Adapted from Giuseppe Cardone's code JUnidecode, which was adapted from Sean M. Burke's Perl code Unidecode.
		StringBuilder after = new StringBuilder();
	    String[] unicodeBlock04 = null;
	    for (int i = 0; i < before.length(); i++) {
	        int codepoint = before.codePointAt(i);
	        int hi = (codepoint >> 8) & 0xff;
	        int low = codepoint & 0xff;
	        
	        if (hi == 0x04) {
	        	if (unicodeBlock04 == null) unicodeBlock04 = getBlock04();
	        	if (low < unicodeBlock04.length) after.append(unicodeBlock04[low]);
	        	else after.appendCodePoint(codepoint);
            } else {
            	after.appendCodePoint(codepoint);
            }
	    }
	    return after.toString();
	}

	private static String[] getBlock04() {
		return new String[]{
		        "Ie", // 0x00
		        "Io", // 0x01
		        "Dj", // 0x02
